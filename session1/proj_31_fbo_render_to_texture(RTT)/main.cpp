...
;
	...
	GLuint fbo = CreateFrameBufferObject(colorBuffer, depthBuffer, width, height); //这个函数相当于，执行之后，一次性生成3个东西给我们，fbo，颜色纹理id，深度纹理id
	...
	unsigned int quadIndices[] = { 0,1,2, 2,3,0 }; //两个三角面
	...
	glBindVertexArray(quadVAO);// 绑定VAO
	...
	glEnableVertexAttribArray(texcoordLocation);
	glVertexAttribPointer(texcoordLocation, 2, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(3 * sizeof(float)));
	glEnableVertexAttribArray(normalLocation);
	...
	while (true)
	{
		...
		// 到这里 内容已经绘制到了fbo中
		glBindFramebuffer(GL_FRAMEBUFFER, 0);// 再次切换到屏幕缓冲来，这样我们想把fbo中的内容，通过四边形在屏幕中呈现就可以做到了
		glViewport(0, 0, width, height);
		glClearColor(0.2f, 0.2f, 0.2f, 1.0f);// 清除颜色为红色， 
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

		glUseProgram(program);

		// 我们构建好矩阵
		glm::mat4 modelQuad = glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, 1.0f, 1.0f));
		glm::mat4 viewQuad = glm::mat4(1.0f);
		glm::mat4 projQuad = glm::mat4(1.0f);
		glm::mat4 normalQuad = glm::mat4(1.0f);


		// shader中的变量设置下
		glUniformMatrix4fv(MLocation, 1, GL_FALSE, glm::value_ptr(modelQuad));
		glUniformMatrix4fv(VLocation, 1, GL_FALSE, glm::value_ptr(viewQuad));
		glUniformMatrix4fv(PLocation, 1, GL_FALSE, glm::value_ptr(projQuad));
		glUniformMatrix4fv(NMLocation, 1, GL_FALSE, glm::value_ptr(normalQuad));


		// 指定fbo中的纹理，给到shader中的0号纹理单元，也就是说它的是来自我们fbo中的colorbufer
